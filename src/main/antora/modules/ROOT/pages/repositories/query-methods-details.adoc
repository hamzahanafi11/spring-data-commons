...
[NOTE]
====
Because we treat underscores (`_`) as a reserved character, we strongly advise following standard Java naming conventions (that is, not using underscores in property names but using camel case instead).
====
...
.Path Ambiguities:
In the following sample the arrangement of properties `qCode` and `q` leading to a property called `code` create an ambiguity for the path `QCode`.
```java
record Container(String qCode, Code q) {}
record Code(String code) {}
```
Since a direct match on a property trumps any potential nested path the algorithm favours the `qCode` field.
Deliminator usage `Q_Code` is required to select the `code` field in `q` .
====
